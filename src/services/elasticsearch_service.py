...
class ElasticsearchService:
    ...
    async def index_pages(self, pages: list, doc_title: str):
        ...
        try:
            ...
            logger.info(
                f"Successfully indexed {success} pages to Elasticsearch. Failed: {len(failed) if isinstance(failed, list) else failed}"
            )
        except Exception as e:
    ...
    async def search(self, query: str, limit: int = 5) -> list[uuid.UUID]:
        ...
        try:
            response = await self._client.search(index=settings.ES_INDEX_NAME, body=search_query)
    ...
    async def delete_by_document_id(self, doc_id: uuid.UUID):
        ...
        query: dict = {"query": {"term": {"document_id": str(doc_id)}}}
        try:
            response = await self._client.delete_by_query(index=settings.ES_INDEX_NAME, body=query, refresh=True)
    ...
    async def clear_index(self, index_name: str | None = None):
        ...
        try:
            await self._client.delete_by_query(index=index_name, body=query, refresh=True)
