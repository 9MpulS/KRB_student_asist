...
class RAGService:
    ...
    async def _get_relevant_chunks_vec(
        self, 
    ...
    async def _get_relevant_chunks_fts(
        self, 
        query: str, 
        limit: int = 5
    ) -> List[Chunk]:
        ...
        try:
            ...
            chunks = await self.chunk_repo.search_text(
                query=query,
                limit=limit
            )
    ...
    async def _get_relevant_pages_es(
        self, 
        query: str, 
    ...
    async def retrieve_chunks(self, question: str) -> List[Chunk]:
        ...
        vec_chunks = await self._get_relevant_chunks_vec(
            question_embedding, 
            limit=settings.RETRIEVAL_LIMIT_VEC
        )
        fts_chunks = await self._get_relevant_chunks_fts(
            fts_query, 
            limit=settings.RETRIEVAL_LIMIT_FTS
        )
    ...
    async def ask_question(self, question: str) -> QuestionResponse:
        ...
        if not retrieved_chunks:
            ...
            return QuestionResponse(
                answer="На жаль, я не знайшов достатньо інформації у базі знань, щоб впевнено відповісти на ваше запитання. Спробуйте уточнити запит.",
                sources=[],
                context=None
            )
        ...
        return QuestionResponse(
            answer=answer,
            sources=sources,
            context=context if settings.DEBUG else None
        )
