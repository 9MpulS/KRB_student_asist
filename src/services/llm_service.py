    ...
    async def rerank_chunks(self, query: str, chunks: list) -> list:
        ...
        try:
            with open("reranking_log.txt", "a", encoding="utf-8") as f:
                ...
                f.write(f"Знайдено: {len(scored_chunks)}, Відібрано (>=0.8): {len(filtered_chunks)}, Обрано для RAG: {len(final_selection)}\n")
        ...
        for chunk, score in final_selection:
            logger.info(f"Rerank top result (selected): {score} - {chunk.page.document.title if chunk.page else 'Unknown'}")
