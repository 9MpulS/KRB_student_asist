...
from src.db.models import Base, Chunk, Document, Page
...
class PageRepository(BaseRepository[Page]):
    ...
    async def get_all_pages_for_migration(self) -> list[Page]:
        ...
        """
        try:
            stmt = select(Page).options(selectinload(Page.document))
    ...
    async def get_pages_by_ids(self, page_ids: list[uuid.UUID]) -> list[Page]:
        ...
        try:
            stmt = select(Page).options(selectinload(Page.document)).where(Page.id.in_(page_ids))
...
class ChunkRepository(BaseRepository[Chunk]):
    ...
    async def get_all_chunks_for_migration(self) -> list[Chunk]:
        ...
        """
        try:
            stmt = select(Chunk).options(selectinload(Chunk.page).selectinload(Page.document))
